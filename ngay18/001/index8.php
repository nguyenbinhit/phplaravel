...
if (isset($_POST) && count($_POST) > 0) {
    ...
    if (isset($_POST["email"]) && (strlen($_POST["email"]) > 0) ) {
        ...
        // nhập email
        $patternEmail = '/^[a-zA-Z0-9]{3,20}+@[^ \t\r\n]{3,5}+\.([a-z]{2,4}|d+)$/i';
        if (preg_match($patternEmail, $email) != 1) {
            // sai định dạng
            $errorAll["email"] = "Email sai định dạng";
        }
    } else {
        // không tồn tại email hay độ dài nhỏ hơn bằng 0
    ...
    if (isset($_POST["sdt"]) && (strlen($_POST["sdt"]) > 0) ) {
        ...
        // nhập sdt
        $patternSDT = '/^\d{10,11}$/i';
        if (preg_match($patternSDT, $sdt) != 1) {
            // sai định dạng
            $errorAll["sdt"] = "SDT sai định dạng";
        }
    } else {
        // không tồn tại email hay độ dài nhỏ hơn bằng 0
